...
pub async fn login(
    ...
    submit_login(client, context, username.into(), password.as_ref(), captcha_answer).await
}
...
pub fn encrypt_password(password: &str, salt: &str) -> String {
    ...
    let ciphertext = cipher.encrypt_padded_vec_mut::<Pkcs7>(
        ("a".repeat(64) + password).into_bytes().as_slice(),
    );
